...
export class TourController {
    getListTour(req, res, next) {
        ...
        if (term) {
            query = { $or: [{ title: { $regex: term, $options: 'i' } }, { content: { $regex: term, $options: 'i' } }] };
        }
    ...
    getCreateTourPage(req, res, next) {
        res.render('tours/new', { tour: {} });
    }

    async getEditTourPage(req, res, next) {
    ...
    async getReserveTourPage(req, res, next) {
        const tour = await Tour.findById(req.params.id);
    ...
    async createTourCourse(req, res, next) {
        const tour = await Tour.findById(req.params.id);
    ...
    async createTourReview(req, res, next) {
        const tour = await Tour.findById(req.params.id);
    ...
    async createTourReserve(req, res, next) {
        const tour = await Tour.findById(req.params.id);
        const user = req.user;
